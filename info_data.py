...
requisites = """
              В данный момент доступны следующие способы:\n
              \tНомер карты (Сбер, СБП): 2202201869695284\n
              При переводе просьба указывать коментарий "neurotool".\n
              Планируется подключение GPT4 со всеми фишками.
              Стоимость месячного подключения стоит 20$. Прогресс сбора средств: 800/2800.
              ...
              подключает поток видео к проекту и настраивает отправку сообщений при
...
update_info = """
               От самых новых:\n
               \t26.03.2024 "neurotool_v1.1.3"
               Произведён рефактор кода, добавлена функция выбора языка в GPT 3.5. Исправлена ошибка 
               добавления сценария. В данный момент работает в тестовом формате через загрузку txt файла.
               Добалена функция загрузки фотографии в YOLO по ссылке на изображение.\n
               \t28.02.2024 "neurotool_v1.1.2"
               Исправлено оторбажение текста. Добавлена политика использования.\n
               \t27.02.2024 "neurotool_v.1.1.1"
               Незначительно обновлён интерфейс, исправлена проблема перекрытия поля отправки кнопкой стримлит
               ...
               Зарождение проекта. Первая рабочая проба пера, представляющая из
...
# Переменная, в которой содержится информация о сохранности данных пользователя и представленны ссылки на документы с политикой использования интегрированных сервисов
term_of_use = """
            ...
            В этом Вы можете убедиться, обратившись к коду проекта на github.\n
            ...
            \tНе подразумевает политики использования: YOLO8
              """
