...
show_start_page = True  # Булева переменная для определения необходимости отображения информации только стартовой страницы (информация о проекте и заголовок)
show_info = False  # Булева переменная для определения необходимости отображения подробной информации о проекте. Она используется в info_func и нужна чтобы изначально более подробная информация не отображалась, но при необходимости  её можно было отобразить, кликнув на checkbox
...
if selected_model == "GPT_3.5_turbo":  # Если выбран GPT_3.5_turbo
    ...
    show_start_page = True  # Иначе показ информации со стартовой страницы сохраняется
