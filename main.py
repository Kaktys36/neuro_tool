...
if show_start_page: # Если переменная True, то
    ...
    st.subheader('''
                 ...
                 ''') # И этот подзаголовок
    info_func(info, show_info, show_start_page) # Функция для отображения подробной информации. Получает переменную info (список с переменными, в которых информация разбита на логические части), булевую переменную show_info для определения необходимости отображения/скрытия подробностей и show_start
