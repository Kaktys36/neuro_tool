...
class ChatBot:
    ...
    def run(self): # Функция запуска модели для вызова из main
        ...
        scenario_examples = [
            'Пример. Запомни: отыгрывай роль пирата. Ты сражаешься с кораблём-призраком. Ты будешь говорить что ты можешь сделать, а я буду тебе помогать, чтобы ты вышел победителем', 
            'Пример. Запомни: представь, что ты астронавт, который потерпел крушение на Марсе. Ты будешь предлагать варианты дальнейших действий, а я направлять тебя',
            'Пример. Запомни: до этого мы говорили о детских мечтах. Представь, что ты сказал, что мечтал в детстве стать киборгом.',
            'Пример. Запомни: проконсультируй меня как профессиональный садовод. Я хочу вырастить несколько расстений мне нужны подробные инструкции.',
            'Пример. Запомни: выполни роль киберспортивного тренера. Я хочу знать всё о том, как подноть свой MMR.',
        ...
        if st.sidebar.checkbox("Добавить сценарий"):
            ...
            st.sidebar.markdown('''
                            ...
                            ''')
            scenario = st.text_input(label='123', value='rand_scenario')
            if scenario:
