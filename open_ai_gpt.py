...
class ChatBot:
    ...
    def run(self):  # Функция запуска модели для вызова из main
        ...
        if st.sidebar.button('Включить ответы только на английском.'):
        ...
        if st.sidebar.toggle('Добавить сценарий'):
            if st.button('Информация о сценарии'):
                st.markdown('''
                       ...
                       (обычный блокнот) и после этого нажать кнопку "Отправить сценарий". При составлении сценария пишите от имени GPT:\n
                       Пример: Я виртуальный помощник, отыгрывающий роль программиста-сеньора.
                       Пример: Я капитан корабля, на который напал корабль-призрак. Помоги мне выйти с победой из сложившейся ситуации. мы играем в ролевую игру.
                        ''')
